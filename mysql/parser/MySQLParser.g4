...
alterStatement
    : ALTER_SYMBOL (
        ...
        | alterEvent
        | alterTablespace
        | {p.isServerVersionGe80014()}? alterUndoTablespace
        | alterLogfileGroup
        | alterServer
...
alterListItem
    ...
    | DROP_SYMBOL (
        ...
        | keyOrIndex indexRef
        | {p.isServerVersionGe80017()}? CHECK_SYMBOL identifier
        | {p.isServerVersionGe80019()}? CONSTRAINT_SYMBOL identifier
    )
    | DISABLE_SYMBOL KEYS_SYMBOL
    | ENABLE_SYMBOL KEYS_SYMBOL
    | ALTER_SYMBOL COLUMN_SYMBOL? columnInternalRef (
        SET_SYMBOL DEFAULT_SYMBOL (
            {p.isServerVersionGe80014()}? exprWithParentheses
            | signedLiteralOrNull
        )
        | DROP_SYMBOL DEFAULT_SYMBOL
        | {p.isServerVersionGe80024()}? SET_SYMBOL visibility
    )
    | ALTER_SYMBOL INDEX_SYMBOL indexRef visibility
    | {p.isServerVersionGe80017()}? ALTER_SYMBOL CHECK_SYMBOL identifier constraintEnforcement
    | {p.isServerVersionGe80019()}? ALTER_SYMBOL CONSTRAINT_SYMBOL identifier constraintEnforcement
    ...
    | CONVERT_SYMBOL TO_SYMBOL charset (
        {p.isServerVersionGe80014()}? DEFAULT_SYMBOL
        | charsetName
    ) collate?
    | FORCE_SYMBOL
...
alterTablespace
    : TABLESPACE_SYMBOL tablespaceRef (
        ...
        | RENAME_SYMBOL TO_SYMBOL identifier
        | {p.isServerVersionGe80014()}? alterTablespaceOptions
    )
    ;
...
alterTablespaceOption
    ...
    | tsOptionWait
    | tsOptionEncryption
    | {p.isServerVersionGe80024()}? tsOptionEngineAttribute
    ;
...
createStatement
    : CREATE_SYMBOL (
        ...
        | createEvent
        | createRole
        | {p.isServerVersionGe80011()}? createSpatialReference
        | {p.isServerVersionGe80014()}? createUndoTablespace
    )
    ;
...
createDatabaseOption
    : defaultCharset
    | defaultCollation
    | {p.isServerVersionGe80016()}? defaultEncryption
    ;
...
storedRoutineBody
    : compoundStatement
    | {p.isStoredRoutineBody()}? AS_SYMBOL routineString
    ;
...
routineOption
    ...
    | option = LANGUAGE_SYMBOL (
        SQL_SYMBOL
        | {p.isServerVersionGe80032()}? identifier
    )
...
tsDataFileName
    ...
    | {p.isServerVersionGe80014()}? (ADD_SYMBOL tsDataFile)? // now optional
    ;
...
tablespaceOption
    ...
    | {p.isServerVersionGe80014()}? tsOptionEncryption
    ;
...
dropStatement
    : DROP_SYMBOL (
        ...
        | dropView
        | dropRole
        | {p.isServerVersionGe80011()}? dropSpatialReference
        | {p.isServerVersionGe80014()}? dropUndoTablespace
    )
    ;
...
deleteStatement
    : withClause? DELETE_SYMBOL deleteStatementOption* (
        FROM_SYMBOL (
            ...
            | tableRef ({p.isServerVersionGe80017()}? tableAlias)? partitionDelete? whereClause? orderClause? simpleLimitClause?
            // Single table delete.
        )
...
valuesReference
    : {p.isServerVersionGe80018()}? AS_SYMBOL identifier columnInternalRefList?
    ;
...
loadFrom
    : {p.isServerVersionGe80200()}? FROM_SYMBOL
    ;

loadSourceType
    : INFILE_SYMBOL
    | {p.isServerVersionGe80200()}? (URL_SYMBOL | S3_SYMBOL)
    ;

sourceCount
    : {p.isServerVersionGe80200()}? (
...
sourceOrder
    : {p.isServerVersionGe80200()}? IN_SYMBOL PRIMARY_SYMBOL KEY_SYMBOL ORDER_SYMBOL
    ;
...
loadAlgorithm
    : {p.isServerVersionGe80200()}? ALGORITHM_SYMBOL EQUAL_OPERATOR BULK_SYMBOL
    ;

loadParallel
    : {p.isServerVersionGe80200()}? PARALLEL_SYMBOL EQUAL_OPERATOR INT_NUMBER
    ;

loadMemory
    : {p.isServerVersionGe80200()}? MEMORY_SYMBOL EQUAL_OPERATOR sizeNumber
    ;
...
selectStatementWithInto
    ...
    | {p.isSelectStatementWithInto()}? queryExpressionParens intoClause
    ;
...
queryExpressionBody
    ...
    (
        (
            UNION_SYMBOL
            | {p.isServerVersionGe80031()}? (EXCEPT_SYMBOL | INTERSECT_SYMBOL)
...
queryPrimary
    : querySpecification
    | {p.isServerVersionGe80019()}? tableValueConstructor
    | {p.isServerVersionGe80019()}? explicitTable
    ;
...
qualifyClause
    : {p.isServerVersionGe80200()}? QUALIFY_SYMBOL expr
    ;
...
groupByClause
    ...
    | {p.isServerVersionGe80032()}? GROUP_SYMBOL BY_SYMBOL (
        ROLLUP_SYMBOL
        | CUBE_SYMBOL
...
lockingClauseList
    : {p.isServerVersionGe80031()}? lockingClause+
    ;
...
tableReference
    ...
    (
        tableFactor
        // ODBC syntax
        | OPEN_CURLY_SYMBOL ({p.isServerVersionGe80017()}? identifier | OJ_SYMBOL) escapedTableReference CLOSE_CURLY_SYMBOL
    ) joinedTable*
    ;
...
tableFactor
    ...
    | {p.isServerVersionGe80004()}? tableFunction
    ;
...
derivedTable
    ...
    | {p.isServerVersionGe80014()}? LATERAL_SYMBOL subquery tableAlias? columnInternalRefList?
    ;
...
jtColumn
    ...
    | identifier dataType ({p.isServerVersionGe80014()}? collate)? EXISTS_SYMBOL? PATH_SYMBOL textStringLiteral
...
tableAlias
    : (AS_SYMBOL | {p.isServerVersionGe80017()}? EQUAL_OPERATOR)? identifier
    ;
...
masterOrBinaryLogsAndGtids
    : MASTER_SYMBOL
    | {p.isServerVersionGe80032()}? BINARY_SYMBOL LOGS_SYMBOL AND_SYMBOL GTIDS_SYMBOL
    ;
...
changeReplicationSource
    : MASTER_SYMBOL
    | {p.isServerVersionGe80024()}? REPLICATION_SYMBOL SOURCE_SYMBOL
    ;
...
sourceDefinition
    ...
    | {p.isServerVersionGe80024()}? SOURCE_CONNECTION_AUTO_FAILOVER_SYMBOL EQUAL_OPERATOR real_ulong_number
    | {p.isServerVersionGe80024()}? ASSIGN_GTIDS_TO_ANONYMOUS_TRANSACTIONS_SYMBOL EQUAL_OPERATOR
        assignGtidsToAnonymousTransactionsDefinition
    | {p.isServerVersionGe80027()}? GTID_ONLY_SYMBOL EQUAL_OPERATOR real_ulong_number
    | sourceFileDef
    ;
...
cloneStatement
    : CLONE_SYMBOL (
        ...
        | {p.isServerVersionGe80014()}? INSTANCE_SYMBOL FROM_SYMBOL user COLON_SYMBOL ulong_number IDENTIFIED_SYMBOL BY_SYMBOL
...
alterUserStatement
    : ALTER_SYMBOL USER_SYMBOL ifExists? (
        (
            {p.isServerVersionGe80014()}? createUserList
            | {p.isServerVersionGe80014()}? alterUserList
        ) createUserTail
        | userFunction (
...
alterUser
    : {p.isServerVersionLt80025()}? oldAlterUser
    | {p.isServerVersionGe80025()}? (
        user (
...
createUserTail
    : requireClause? connectOptions? accountLockPasswordExpireOptions* (
        {p.isServerVersionGe80024()}? userAttributes
    )?
    ;
...
accountLockPasswordExpireOptions
    ...
    | PASSWORD_SYMBOL (
        ...
        | {p.isServerVersionGe80014()}? REQUIRE_SYMBOL CURRENT_SYMBOL (
            DEFAULT_SYMBOL
            | OPTIONAL_SYMBOL
        )?
    )
...
grantTargetList
    : {p.isServerVersionLt80011()}? createUserList
    | {p.isServerVersionGe80011()}? userList
    ;
...
versionedRequireClause
    : {p.isServerVersionLt80011()}? requireClause
    ;
...
revokeStatement
    : REVOKE_SYMBOL ({p.isServerVersionGe80031()}? ifExists)? (
    ...
    ) ({p.isServerVersionGe80031()}? ignoreUnknownUser)?
    ;
...
grantIdentifier
    ...
    | tableRef
    | {p.isServerVersionGe80017()}? schemaRef DOT_SYMBOL tableRef
    ;
...
histogramAutoUpdate
    : {p.isServerVersionGe80200()}? (MANUAL_SYMBOL | AUTO_SYMBOL) UPDATE_SYMBOL
    ;

histogramUpdateParam
    : histogramNumBuckets? histogramAutoUpdate?
    | {p.isServerVersionGe80031()}? USING_SYMBOL DATA_SYMBOL textStringLiteral
    ;

histogramNumBuckets
    : {p.isServerVersionGe80200()}? WITH_SYMBOL INT_NUMBER BUCKETS_SYMBOL
    ;
...
installSetValueList
    : {p.isServerVersionGe80032()}? SET_SYMBOL installSetValue (
...
startOptionValueList
    ...
    | PASSWORD_SYMBOL (FOR_SYMBOL user)? equal (
        ...
        | textString replacePassword? retainCurrentPassword?
        | {p.isServerVersionLt80014()}? PASSWORD_SYMBOL OPEN_PAR_SYMBOL textString CLOSE_PAR_SYMBOL
    )
    | {p.isServerVersionGe80018()}? PASSWORD_SYMBOL (FOR_SYMBOL user)? TO_SYMBOL RANDOM_SYMBOL replacePassword?
        retainCurrentPassword?
    ;
...
optionValueNoOptionType
    ...
    | NAMES_SYMBOL (
        equal expr
        | charsetName collate?
        | {p.isServerVersionGe80011()}? DEFAULT_SYMBOL
    )
    ;
...
showParseTreeStatement
    : {p.isServerVersionGe80100()}? SHOW_SYMBOL PARSE_TREE_SYMBOL simpleStatement
    ;
...
utilityStatement
    ...
    | helpCommand
    | useCommand
    | {p.isServerVersionGe80011()}? restartServer
    ;
...
explainStatement
    : (EXPLAIN_SYMBOL | DESCRIBE_SYMBOL | DESC_SYMBOL) explainOptions? (
        {p.isServerVersionGe80032()}? FOR_SYMBOL DATABASE_SYMBOL textOrIdentifier
    )? explainableStatement
    ;

explainOptions
    : FORMAT_SYMBOL EQUAL_OPERATOR textOrIdentifier (
        {p.isServerVersionGe80032()}? explainInto
    )?
    | {p.isServerVersionLt80012()}? EXTENDED_SYMBOL
    | {p.isServerVersionGe80018()}? ANALYZE_SYMBOL
    | {p.isServerVersionGe80019()}? ANALYZE_SYMBOL FORMAT_SYMBOL EQUAL_OPERATOR textOrIdentifier
    ;
...
predicate
    : bitExpr (
        ...
        | {p.isServerVersionGe80017()}? MEMBER_SYMBOL OF_SYMBOL? simpleExprWithParentheses
...
arrayCast
    : {p.isServerVersionGe80017()}? ARRAY_SYMBOL
    ;
...
windowFunctionCall
    ...
    | NTILE_SYMBOL (
        ...
        | {p.isServerVersionLt80024()}? simpleExprWithParentheses
    ) windowingClause
...
tablesampleClause
    : {p.isServerVersionGe80200()}? TABLESAMPLE_SYMBOL samplingMethod OPEN_PAR_SYMBOL samplingPercentage CLOSE_PAR_SYMBOL
    ;
...
leadLagInfo
    : COMMA_SYMBOL (
        ulonglongNumber
        | PARAM_MARKER
        | {p.isServerVersionGe80024()}? stableInteger
    ) (COMMA_SYMBOL expr)?
    ;
...
runtimeFunctionCall
    ...
    | {p.isServerVersionGe80032()}? LOG_SYMBOL OPEN_PAR_SYMBOL expr (
        COMMA_SYMBOL expr
    )? CLOSE_PAR_SYMBOL
    ...
    | {p.isServerVersionLt80011()}? PASSWORD_SYMBOL exprWithParentheses
...
lvalueVariable
    : (
        ...
        | {p.isServerVersionGe80017()}? lValueIdentifier dotIdentifier?
    )
...
castType
    ...
    | DATE_SYMBOL
    | {p.isServerVersionGe80024()}? YEAR_SYMBOL
    | TIME_SYMBOL typeDatetimePrecision?
    | DATETIME_SYMBOL typeDatetimePrecision?
    | DECIMAL_SYMBOL floatOptions?
    | JSON_SYMBOL
    | {p.isServerVersionGe80017()}? realType
    | {p.isServerVersionGe80017()}? FLOAT_SYMBOL standardFloatOptions?
    | {p.isServerVersionGe80027()}? (
        POINT_SYMBOL
        | LINESTRING_SYMBOL
        | POLYGON_SYMBOL
        | MULTIPOINT_SYMBOL
...
checkOrReferences
    : {p.isServerVersionLt80016()}? checkConstraint
    | references
    ;
...
constraintEnforcement
    : {p.isServerVersionGe80017()}? NOT_SYMBOL? ENFORCED_SYMBOL
    ;
...
columnAttribute
    ...
    | {p.isServerVersionGe80014()}? NOT_SYMBOL SECONDARY_SYMBOL
    | value = DEFAULT_SYMBOL (
        nowOrSignedLiteral
        | {p.isServerVersionGe80013()}? exprWithParentheses
    )
    ...
    | {p.isServerVersionGe80017()}? constraintName? checkConstraint
    | {p.isServerVersionGe80017()}? constraintEnforcement
    | {p.isServerVersionGe80024()}? value = ENGINE_ATTRIBUTE_SYMBOL EQUAL_OPERATOR? jsonAttribute
    | {p.isServerVersionGe80024()}? value = SECONDARY_ENGINE_ATTRIBUTE_SYMBOL EQUAL_OPERATOR? jsonAttribute
    | {p.isServerVersionGe80024()}? visibility
    ;
...
keyPartOrExpression
    ...
    keyPart
    | {p.isServerVersionGe80013()}? exprWithParentheses direction?
    ;
...
commonIndexOption
    ...
    | COMMENT_SYMBOL textLiteral
    | visibility
    | {p.isServerVersionGe80024()}? ENGINE_ATTRIBUTE_SYMBOL EQUAL_OPERATOR? jsonAttribute
    | {p.isServerVersionGe80024()}? SECONDARY_ENGINE_ATTRIBUTE_SYMBOL EQUAL_OPERATOR? jsonAttribute
    ;
...
charsetName
    : textOrIdentifier
    | BINARY_SYMBOL
    | {p.isServerVersionLt80011()}? DEFAULT_SYMBOL
    ;

collationName
    : textOrIdentifier
    | {p.isServerVersionLt80011()}? DEFAULT_SYMBOL
    | {p.isServerVersionGe80018()}? BINARY_SYMBOL
    ;
...
createTableOption
    ...
    | {p.isServerVersionGe80014()}? option = SECONDARY_ENGINE_SYMBOL equal? (
        NULL_SYMBOL
        | textOrIdentifier
    )
    ...
    | {p.isServerVersionGe80024()}? option = START_SYMBOL TRANSACTION_SYMBOL
    | {p.isServerVersionGe80024()}? option = ENGINE_ATTRIBUTE_SYMBOL EQUAL_OPERATOR? jsonAttribute
    | {p.isServerVersionGe80024()}? option = SECONDARY_ENGINE_ATTRIBUTE_SYMBOL EQUAL_OPERATOR? jsonAttribute
    | {p.isServerVersionGe80024()}? tsOptionAutoextendSize
    ;
...
pureIdentifier
    ...
    | {p.isPureIdentifier()}? DOUBLE_QUOTED_TEXT
    ;
...
real_ulonglong_number
    : INT_NUMBER
    | {p.isServerVersionGe80017()}? HEX_NUMBER
    | ULONGLONG_NUMBER
    | LONG_NUMBER
    ;
...
signedLiteralOrNull
    : signedLiteral
    | {p.isServerVersionGe80024()}? nullAsLiteral
    ;
...
textStringLiteral
    ...
    | {p.isTextStringLiteral()}? value = DOUBLE_QUOTED_TEXT
    ;
...
roleKeyword
    : {p.isServerVersionLt80017()}? (roleOrLabelKeyword | roleOrIdentifierKeyword)
    | (
        identifierKeywordsUnambiguous
        | identifierKeywordsAmbiguous2Labels
        | identifierKeywordsAmbiguous4SystemVariables
...
roleOrLabelKeyword
    ...
    | {p.isServerVersionGe80014()}? ADMIN_SYMBOL
    ;